...
- **项目作者**：玖念
- **项目背景**：在寻找众多图床项目后，发现它们都不太符合我的需求，于是决定自己开发一个并开源，希望能帮到有同样需求的人。后续会根据反馈持续优化与完善。 本项目由 AI 辅助创建，采用开源项目标准规范组织代码与文档，便于二次开发与维护。
...
├── install.php               # 一键安装向导（★ 安装完成后会自动生成锁文件“install.lock”再次打开会自动禁止访问 不要删除该文件“install.php”）
├── login.php                 # 登录
├── register.php              # 注册
├── logout.php                # 退出登录
...
4. 浏览器访问 `https://你的域名/install.php`，按向导完成：
   - 自动建库、建表并写入初始数据
   - 创建管理员账号
5. 安装完成后会自动生成 `install.lock`，安装向导已被锁定，无需手动删除 `install.php`。
